...
2. **🔄 التعرف اللحظي والتحديث الحي للأزرار (Real-Time Live UI Updates)**:
   - الاستماع المباشر لحدث `ChatMemberUpdated`: بمجرد انضمام المستخدم للقناة في تليجرام، يتم تعديل رسالة البوت تلقائياً وحذف زر القناة التي اشترك بها للتو، أو إتاحة البوت والترحيب به فوراً بمجرد إكمال جميع القنوات دون الحاجة لضغط زر التحقق!
   - إلغاء الكاش فوراً عند مغادرة المستخدم للقناة لمنع التحايل.
3. **📢 دعم قنوات طلبات الانضمام (Join Requests Tracking)**:
   - الاستماع التلقائي لطلبات الانضمام (`ChatJoinRequest`) واعتبار المستخدم مشتركاً بمجرد إرسال الطلب (اختياري).
4. **🎯 التدوير الذكي ونظام الحصص (Smart Rotation & Join Quotas)**:
...
7. **🛡️ التعافي الذاتي (Self-Healing Fallback)**:
...
9. **🤖 دليل شامل للوكلاء الذكيين (`AGENTS.md`)**:
   - ملف توثيق مخصص للوكلاء الاصطناعيين والمطورين يشرح معمارية الحزمة وهيكلة مفاتيح Redis بالتفصيل.
...
async def main():
    ...
    # ملاحظة: احرص على تضمين chat_member و chat_join_request لاستقبال التحديثات اللحظية
